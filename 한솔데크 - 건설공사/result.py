...
def load_pdfs_from_directory(directory):
    """디렉토리 내 모든 PDF 파일을 로드하여 텍스트로 변환"""
    pdf_texts = []
...
# PDF 데이터 로드
pdf_texts = load_pdfs_from_directory("/home/elicer/Dacon/건설안전지침")
print(f"총 {len(pdf_texts)}개의 PDF 파일이 로드됨")

# PDF 텍스트 청킹
text_splitter = RecursiveCharacterTextSplitter(
    chunk_size=1000,  # 1000자 단위로 쪼갬
...
print(f"PDF 데이터에서 총 {len(pdf_chunks)}개의 청크 생성됨")
...
train_documents = [
    f"Q: {row['question']}\nA: {row['answer']}"
    for _, row in combined_training_data.iterrows()
]
...
# PDF 데이터 추가
vector_store.add_texts(pdf_chunks)

# Retriever 업데이트
retriever = vector_store.as_retriever(search_type="similarity", search_kwargs={"k": 5})

print("CSV 및 PDF 데이터가 벡터 스토어에 추가됨")
